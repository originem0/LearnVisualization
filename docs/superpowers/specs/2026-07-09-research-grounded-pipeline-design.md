...
- 服务器实际为墙内 VM（腾讯云），直连 zh/en 维基与搜索引擎不可达且 DNS 被污染；设计期的"新加坡无墙"结论是测试被 shell 代理静默转发的误判。实施修正：research 抓取走本机代理（`AGENT_RESEARCH_PROXY`，systemd 注入），LLM 中转调用保持直连。DDG 在该代理出口下常被限流，维基为主要语料源。
...
发布门设计修订：原方案的 waiting_review 人工门在实现中是"审核剧场"——审核界面看不到正文、不能编辑，只能批准或重生成，用户等待成本高而判断输入为零。既然机器防线（引文逐字校验、证据锚定、评审、全课终检）已经承担实质把关，auto-publish 恢复，人工判断后移为发布后策展（读线上渲染成品，删除或重生成）。DESIGN.md"人类判断不可取消"以策展形式保留。

配套修订：
- 重试语义：从 compose 重试保留已通过章节的检查点（只重写失败章）；research/plan 被重置时检查点才作废
- 新增本地碎块化闸（narrative >24 块或 text 块平均过短 → 重写反馈），来自首门生成课程 c01/c03 的实际缺陷
- 逐章 4 次带反馈重写即"针对性局部重生成"；证据库上限 40 条；章节输出预算 16k tokens
