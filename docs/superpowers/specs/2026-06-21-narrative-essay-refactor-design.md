...
6. **"事实脊柱"的素材来源（防八股成败关键）**：现有 plan 的 `researchSummary` 是 LLM 自产、非真实外部研究。对模型熟悉的主题（LLM、Go、Python）其参数知识够用；对冷门主题，自产"事实"可能是幻觉，比空转更糟。是否在 plan 前接入真实研究（仓库已有 `research/`、`DEEP_RESEARCH_PROMPTS.md` 及 crawl4ai/firecrawl 能力），还是按主题熟悉度分流——实现时定。
